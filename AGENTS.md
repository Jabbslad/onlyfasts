...
1. **Update the changelog** in `onlyfasts/src/content/changelog.ts`:
...
2. **Update the semver version** in `onlyfasts/src/content/changelog.ts` and ensure it follows semantic versioning:
...
3. **Verify before committing**:
   - Run `npx tsc --noEmit` from the `onlyfasts/` directory
   - Run `npx vitest run` from the `onlyfasts/` directory
